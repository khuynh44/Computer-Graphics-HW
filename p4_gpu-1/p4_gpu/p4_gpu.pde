...
void draw() {
  ...
  // endShape();
  float inc = 250.0/30.0;
  float u = 0;
  float v = 0;
  float uvinc = 1.0/30.0;
  
  for (float y = 50; y < 300; y += inc) {
    u = 0;
    for (float x = -300; x < -50; x += inc) {
      beginShape();
      texture(ship_texture);
      vertex(x, y, -300, u, v);
      vertex(x + inc, y, -300, u + uvinc, v);
      vertex(x + inc, y + inc, -300, u + uvinc, v + uvinc);
      vertex(x, y + inc, -300, u, v + uvinc);
      endShape();
      u += uvinc;
    }
    v += uvinc;
